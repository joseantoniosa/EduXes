...
\subsection{Problems and drawbacks}
 From initial plan, there was several changes:
  ...
  Most difficult page: List of assessment by student:
      ...
      Up to four different ideas were tried. At last an ad-hoc object
      was created, Peers, 

 
 Pointedly a comment in code is set with a XXX tag. The most difficult problem
 was to deal with several students three different activities, then I realized a need
 ...
 an object: Peers. Peers object is an object which contains activity id, mark and its weight.
...
Report attendance was difficult to implement, a new global variable 
  was set report\_date  which allow week change.
...
it is tightly related with Attendance. Assessment contains two main elements:
list of activities (by default it was set to 0), and list of students with
a bar


 Initially current date was set by hand (or with a native widget which depends on Android version),
  this was a problem  because user has to deal with month/day/year format. 
My mentor, Manuel Rego, told me about a widget LibrePlan app \cite{LireplanApp} was using: 
